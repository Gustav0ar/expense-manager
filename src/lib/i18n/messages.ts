export const ptBrMessages = {
	'A network error interrupted delivery': 'Um erro de rede interrompeu o envio',
	Access: 'Acesso',
	Actions: 'Ações',
	Active: 'Ativa',
	'Active categories': 'Categorias ativas',
	Add: 'Adicionar',
	'Add a category budget to see its alert threshold here.':
		'Adicione um orçamento de categoria para ver o limite de alerta aqui.',
	'Add options for payment, vendor and cost center.':
		'Adicione opções para pagamento, fornecedor e centro de custo.',
	'Add options for payment, vendor, cost center and category.':
		'Adicione opções para pagamento, fornecedor, centro de custo e categoria.',
	Admin: 'Administrador',
	Alert: 'Alerta',
	'Alert delivery history pagination': 'Paginação do histórico de envios de alertas',
	'Alert thresholds': 'Limites de alerta',
	'All workspace managers': 'Todos os gestores do workspace',
	Analytical: 'Analítico',
	Analysis: 'Análise',
	Amount: 'Valor',
	Appearance: 'Aparência',
	Apply: 'Aplicar',
	Attempts: 'Tentativas',
	'Automatic alerts off': 'Alertas automáticos desativados',
	'Automatic alerts on': 'Alertas automáticos ativados',
	'Automatic email alerts': 'Alertas automáticos por email',
	'Automatic budget alerts disabled.': 'Alertas automáticos de orçamento desativados.',
	'Automatic budget alerts enabled.': 'Alertas automáticos de orçamento ativados.',
	'Automatic rules': 'Regras automáticas',
	Approve: 'Aprovar',
	Approved: 'Aprovada',
	'Approved amount': 'Valor aprovado',
	Archived: 'Arquivado',
	Attachment: 'Comprovante',
	Attachments: 'Comprovantes',
	Attach: 'Anexar',
	Audit: 'Auditoria',
	Authentication: 'Autenticação',
	Back: 'Voltar',
	'Back to login': 'Voltar para login',
	'Below warning threshold': 'Abaixo do limite de alerta',
	'Blocked by provider': 'Bloqueado pelo provedor',
	'Bounce reported by provider': 'Devolução informada pelo provedor',
	Budget: 'Orçamento',
	'Budget alert delivery not found.': 'Envio de alerta de orçamento não encontrado.',
	'Budget alert delivery retried.': 'Nova tentativa do alerta de orçamento enviada.',
	'Budget alert delivery retry failed.': 'Não foi possível repetir o envio do alerta de orçamento.',
	'Budget alert delivery is already in progress.':
		'O envio dos alertas de orçamento já está em andamento.',
	'Budget alert preferences saved.': 'Preferências de alertas de orçamento salvas.',
	'Budget alerts': 'Alertas de orçamento',
	'Budget amount': 'Valor do orçamento',
	'Budget by category': 'Orçamento por categoria',
	'Budget month': 'Mês do orçamento',
	'Budget short': 'Orç.',
	Budgets: 'Orçamentos',
	Cancel: 'Cancelar',
	'Catalog item added successfully.': 'Item adicionado ao cadastro com sucesso.',
	'Catalog item archived successfully.': 'Item arquivado com sucesso.',
	'Catalog item deleted successfully.': 'Item excluído com sucesso.',
	'Catalog item updated successfully.': 'Item atualizado com sucesso.',
	'Catalog type': 'Tipo de cadastro',
	Category: 'Categoria',
	'Category already exists.': 'Categoria já existe.',
	'Category archived successfully.': 'Categoria arquivada com sucesso.',
	'Category created successfully.': 'Categoria criada com sucesso.',
	'Category deleted successfully.': 'Categoria excluída com sucesso.',
	'Category name': 'Nome da categoria',
	'Category restored successfully.': 'Categoria restaurada com sucesso.',
	'Category status': 'Status da categoria',
	'Category updated successfully.': 'Categoria atualizada com sucesso.',
	Categories: 'Categorias',
	'Categories short': 'Cat.',
	Center: 'Centro',
	'Choose who receives threshold and over-budget email alerts.':
		'Escolha quem recebe alertas por email sobre limites e estouro do orçamento.',
	'Click reported by provider': 'Clique informado pelo provedor',
	Clear: 'Limpar',
	'Clear {label}': 'Limpar {label}',
	Closed: 'Fechada',
	'Confirm delete': 'Confirmar exclusão',
	'Code generated in the app': 'Código gerado no app',
	Color: 'Cor',
	Compact: 'Compacto',
	'Configure MFA': 'Configurar MFA',
	Confirm: 'Confirmar',
	'Confirm password': 'Confirmar senha',
	Contains: 'Contém',
	contains: 'contém',
	'Compressing image...': 'Comprimindo imagem...',
	Competency: 'Competência',
	'Competency month': 'Mês de competência',
	Control: 'Controle',
	'Cost center': 'Centro de custo',
	'Cost centers': 'Centros de custo',
	Create: 'Criar',
	'Create account': 'Criar conta',
	'Create recurrence': 'Criar recorrência',
	'Create rule': 'Criar regra',
	'Create workspace': 'Criar workspace',
	CSV: 'CSV',
	'CSV template': 'Modelo CSV',
	Current: 'Atual',
	'Current code': 'Código atual',
	'Current usage for {period}': 'Uso atual em {period}',
	Currency: 'Moeda',
	Dashboard: 'Dashboard',
	Date: 'Data',
	'Date basis': 'Base de data',
	Delete: 'Excluir',
	'Delete expense?': 'Excluir despesa?',
	Description: 'Descrição',
	Details: 'Detalhes',
	'Delivery attempts will appear here after the first alert is triggered.':
		'As tentativas de envio aparecerão aqui após o primeiro alerta ser acionado.',
	'Delivery configuration failed': 'Falha na configuração do envio',
	'Delivery failed': 'Falha no envio',
	'Delivery history': 'Histórico de envios',
	'Delivery timed out': 'O envio expirou',
	'Delivery was rejected': 'O envio foi rejeitado',
	'Device language': 'Idioma do dispositivo',
	'Disable automatic alerts': 'Desativar alertas automáticos',
	'Disable MFA': 'Desativar MFA',
	'Distribution by category': 'Distribuição por categoria',
	Done: 'Concluído',
	Download: 'Download',
	'Download a versioned CSV that can be imported again (up to {count} expenses and 1 MB).':
		'Baixe um CSV versionado que pode ser importado novamente (até {count} despesas e 1 MB).',
	'Download the versioned import template.': 'Baixe o modelo versionado de importação.',
	Edit: 'Editar',
	Email: 'Email',
	'Email delivery': 'Envio de email',
	'Email provider was unavailable': 'O provedor de email estava indisponível',
	Emoji: 'Emoji',
	'Enable automatic alerts': 'Ativar alertas automáticos',
	'Enable alerts': 'Enviar alertas',
	'Enable MFA': 'Ativar MFA',
	End: 'Fim',
	'End date': 'Data final',
	'Escalate when a budget is exceeded': 'Escalar quando o orçamento for excedido',
	Escalation: 'Escalonamento',
	English: 'Inglês',
	Enter: 'Entrar',
	Entries: 'Lançamentos',
	Entity: 'Entidade',
	Events: 'Eventos',
	'Expense Manager': 'Expense Manager',
	Expenses: 'Despesas',
	'Expenses by category': 'Despesas por categoria',
	'Expenses by month': 'Despesas por mês',
	'Expenses by week': 'Despesas por semana',
	'Expenses registered': 'Despesas lançadas',
	'Expenses short': 'Desp.',
	Export: 'Exportar',
	Failed: 'Falhou',
	Failures: 'Falhas',
	Field: 'Campo',
	Filter: 'Filtrar',
	'First page': 'Primeira página',
	File: 'Arquivo',
	Format: 'Formato',
	Frequency: 'Frequência',
	'Follow each category from its warning threshold through the monthly limit.':
		'Acompanhe cada categoria desde o limite de alerta até o limite mensal.',
	Generate: 'Gerar',
	'Generate due': 'Gerar vencidas',
	Group: 'Grupo',
	'Group by': 'Agrupar',
	'Home short': 'Início',
	Import: 'Importar',
	'Import batch is invalid.': 'O lote de importação é inválido.',
	'Import batch not found.': 'Lote de importação não encontrado.',
	'Import expenses': 'Importar despesas',
	'Import failures': 'Falhas da importação',
	'Import preview': 'Prévia da importação',
	'Import preview checksum does not match.': 'O checksum da prévia da importação não confere.',
	'Import preview expired. Upload the file again.':
		'A prévia da importação expirou. Envie o arquivo novamente.',
	'Import preview is invalid.': 'A prévia da importação é inválida.',
	'Import preview not found.': 'Prévia da importação não encontrada.',
	'Import preview ready. Review before confirming.':
		'Prévia da importação pronta. Revise antes de confirmar.',
	'Import row selection is invalid.': 'A seleção de linhas da importação é inválida.',
	Imported: 'Importadas',
	Include: 'Incluir',
	'In progress': 'Em andamento',
	'Initial alert': 'Alerta inicial',
	Installment: 'Parcela',
	'Installment amount': 'Valor da parcela',
	Installments: 'Parcelas',
	Interval: 'Intervalo',
	Internet: 'Internet',
	Invite: 'Convidar',
	Invitations: 'Convites',
	'Items per page': 'Itens por página',
	'Keep the recipient list aligned with manager access.':
		'Mantenha a lista de destinatários alinhada ao acesso de gestores.',
	Language: 'Idioma',
	'Last updated': 'Última atualização',
	'Legacy invitation': 'Convite anterior ao rastreamento',
	'Legacy monthly alert': 'Alerta mensal anterior ao rastreamento',
	'Last page': 'Última página',
	Light: 'Claro',
	List: 'Lista',
	Loading: 'Carregando',
	Login: 'Entrar',
	Logout: 'Sair',
	Manage: 'Gerenciar',
	Manual: 'Manual',
	Marketing: 'Marketing',
	'Manual retry unavailable': 'Nova tentativa manual indisponível',
	Member: 'Membro',
	Members: 'Membros',
	Metadata: 'Metadados',
	Month: 'Mês',
	Monthly: 'Mensal',
	'Monthly alert': 'Alerta mensal',
	'Monthly budget': 'Orçamento mensal',
	'Monthly evolution': 'Evolução mensal',
	Name: 'Nome',
	'New category': 'Nova categoria',
	'New cost center': 'Novo centro de custo',
	'New expense': 'Nova despesa',
	'New payment': 'Novo pagamento',
	'New vendor': 'Novo fornecedor',
	'New workspace': 'Novo workspace',
	Next: 'Próxima',
	'Next page': 'Próxima página',
	'Next page of {items}': 'Próxima página de {items}',
	'No archived category found.': 'Nenhuma categoria arquivada encontrada.',
	'No alert delivery history yet.': 'Ainda não há histórico de envios de alertas.',
	'No attachments added.': 'Nenhum comprovante anexado.',
	'No budget alerts to send.': 'Nenhum alerta de orçamento para enviar.',
	'No budget defined.': 'Nenhum orçamento definido.',
	'No budget thresholds are configured.': 'Nenhum limite de orçamento está configurado.',
	'No category found.': 'Nenhuma categoria encontrada.',
	'No cost center found.': 'Nenhum centro de custo encontrado.',
	'No data': 'Sem dados',
	'No expense found.': 'Nenhuma despesa encontrada.',
	'No expenses imported.': 'Nenhuma despesa importada.',
	'No expenses in the period.': 'Sem despesas no período.',
	'No expenses selected.': 'Nenhuma despesa selecionada.',
	'No eligible managers are available for budget alerts.':
		'Não há gestores elegíveis disponíveis para alertas de orçamento.',
	'No events found.': 'Nenhum evento encontrado.',
	'No goal': 'Sem meta',
	'No goals': 'Sem metas',
	'No payment method found.': 'Nenhum pagamento encontrado.',
	'No recurrence due to generate.': 'Nenhuma recorrência vencida para gerar.',
	'No rule created.': 'Nenhuma regra criada.',
	'No results': 'Nenhum resultado',
	'No search results.': 'Nenhum resultado para a busca.',
	'No usage': 'Sem uso',
	'No vendor found.': 'Nenhum fornecedor encontrado.',
	Notes: 'Notas',
	'Notification settings': 'Configurações de notificações',
	Notifications: 'Notificações',
	Open: 'Aberta',
	'Open {label}': 'Abrir {label}',
	'Open reported by provider': 'Abertura informada pelo provedor',
	Organization: 'Organização',
	'Older deliveries': 'Envios anteriores',
	'Over budget': 'Orçamento excedido',
	Overview: 'Visão geral',
	Owner: 'Owner',
	Page: 'Página',
	Paid: 'Paga',
	Pagination: 'Paginação',
	Password: 'Senha',
	Payment: 'Pagamento',
	'Payment date': 'Data de pagamento',
	'Payment method': 'Forma de pagamento',
	'Payment status': 'Status de pagamento',
	Payments: 'Pagamentos',
	Pending: 'Pendente',
	Period: 'Período',
	Planning: 'Planejamento',
	'Planning short': 'Plano',
	'Portable CSV': 'CSV portável',
	'Portuguese (Brazil)': 'Português (Brasil)',
	Previous: 'Anterior',
	'Previous page': 'Página anterior',
	'Previous page of {items}': 'Página anterior de {items}',
	Priority: 'Prioridade',
	Primary: 'Principal',
	'Provider delivery update': 'Atualização de envio do provedor',
	Queued: 'Na fila',
	Receipt: 'Comprovante',
	Reconciled: 'Conciliada',
	'Recover password': 'Recuperar senha',
	'Recovery codes': 'Recovery codes',
	'Recurring expenses': 'Despesas recorrentes',
	'Recent budget alert activity and provider updates.':
		'Atividade recente de alertas de orçamento e atualizações do provedor.',
	'Registration is currently closed.': 'O cadastro está fechado no momento.',
	Recurrences: 'Recorrências',
	Rejected: 'Rejeitada',
	Reject: 'Rejeitar',
	Remove: 'Remover',
	'Remove budget': 'Remover orçamento',
	'Report CSV': 'CSV do relatório',
	Reports: 'Relatórios',
	'Reports short': 'Rel.',
	Recipients: 'Destinatários',
	Resend: 'Reenviar',
	'Reset password': 'Redefinir senha',
	Result: 'Resultado',
	'Retry delivery': 'Repetir envio',
	'Retry scheduled': 'Nova tentativa agendada',
	Review: 'Revisão',
	'Review status': 'Status da revisão',
	Role: 'Papel',
	Rules: 'Regras',
	Save: 'Salvar',
	'Save notification settings': 'Salvar configurações de notificações',
	'Save language': 'Salvar idioma',
	'Save payment': 'Salvar pagamento',
	'Save password': 'Salvar senha',
	'Save theme': 'Salvar tema',
	Search: 'Buscar',
	Security: 'Segurança',
	Select: 'Selecione',
	'Select at least one alert recipient.': 'Selecione pelo menos um destinatário de alertas.',
	'Select only eligible alert recipients.':
		'Selecione apenas destinatários elegíveis para alertas.',
	'Selected managers': 'Gestores selecionados',
	'Selected recipients ({count})': 'Destinatários selecionados ({count})',
	Send: 'Enviar',
	'Send a follow-up alert after a category crosses its monthly limit.':
		'Envie um alerta adicional quando uma categoria ultrapassar seu limite mensal.',
	'Send alerts only to the people selected below.':
		'Envie alertas apenas para as pessoas selecionadas abaixo.',
	'Send alerts when configured budget thresholds are reached.':
		'Envie alertas quando os limites de orçamento configurados forem atingidos.',
	Sending: 'Enviando',
	Sent: 'Enviado',
	'Sent reported by provider': 'Envio informado pelo provedor',
	Settings: 'Ajustes',
	'Settings short': 'Ajustes',
	Setup: 'Configuração',
	'Show password': 'Mostrar senha',
	'Sign up': 'Cadastrar',
	Start: 'Início',
	'Start date': 'Data inicial',
	Status: 'Status',
	'Spam report received by provider': 'Denúncia de spam recebida pelo provedor',
	System: 'Sistema',
	'Team short': 'Equipe',
	Theme: 'Tema',
	'This budget alert is no longer active.': 'Este alerta de orçamento não está mais ativo.',
	'This delivery cannot be retried.': 'Este envio não pode ser repetido.',
	'This action removes the entry and updates dashboards and reports.':
		'Essa ação remove o lançamento e atualiza os dashboards e relatórios.',
	'This invite belongs to another email.': 'Este convite pertence a outro email.',
	'This recipient is no longer eligible.': 'Este destinatário não é mais elegível.',
	Total: 'Total',
	'Total in period': 'Total no período',
	'Totals registered': 'Totais cadastrados',
	Transaction: 'Lançamento',
	'Transaction date': 'Data da transação',
	Transactions: 'Lançamentos',
	Transfer: 'Trocar',
	Update: 'Atualizar',
	'Unknown status': 'Status desconhecido',
	'Unsubscribe reported by provider': 'Cancelamento de inscrição informado pelo provedor',
	Upload: 'Upload',
	'Uploading attachment...': 'Enviando anexo...',
	User: 'Usuário',
	Users: 'Usuários',
	Value: 'Valor',
	Vendor: 'Fornecedor',
	Vendors: 'Fornecedores',
	'View month': 'Ver mês',
	'Warn at {threshold}': 'Alertar em {threshold}',
	Week: 'Semana',
	'Week starts on': 'Início da semana',
	Weekly: 'Semanal',
	'Weekly average': 'Média semanal',
	Warning: 'Alerta',
	'Warning threshold reached': 'Limite de alerta atingido',
	Workspace: 'Workspace',
	Year: 'Ano',
	Yearly: 'Anual',
	'{category}: {usage} used; warning at {threshold}.':
		'{category}: {usage} usado; alerta em {threshold}.',
	'{count} attempts': '{count} tentativas',
	Monday: 'Segunda-feira',
	Sunday: 'Domingo',
	Dark: 'Escuro',
	Account: 'Conta',
	'Account and audit': 'Conta e auditoria',
	Action: 'Ação',
	Actor: 'Ator',
	All: 'Todos',
	'All categories': 'Todas',
	'All reviews': 'Todas',
	'All payments': 'Todos',
	Archive: 'Arquivar',
	'Archive category': 'Arquivar categoria',
	'Archived item': 'Item arquivado',
	'Archived categories': 'Categorias arquivadas',
	'Archived payment method': 'Pagamento arquivado',
	'Archived vendor': 'Fornecedor arquivado',
	'Archived cost center': 'Centro de custo arquivado',
	'Default category': 'Categoria padrão',
	'Use category column': 'Usar coluna categoria',
	Display: 'Exibição',
	Close: 'Fechar',
	'Support catalogs': 'Cadastros de apoio',
	'Open status': 'Aberta',
	Reason: 'Motivo',
	'Rejection reason': 'Motivo da rejeição',
	'Next run': 'Próximo',
	Pause: 'Pausar',
	Resume: 'Retomar',
	Restore: 'Restaurar',
	'Restore category': 'Restaurar categoria',
	Secret: 'Secret',
	URI: 'URI',
	Enable: 'Ativar',
	'MFA verification': 'Verificação MFA',
	Verification: 'Verificação',
	'Authenticator code or recovery code': 'Código do autenticador ou recovery code',
	Verify: 'Verificar',
	'Forgot password': 'Esqueci minha senha',
	'Hide password': 'Ocultar senha',
	'I already have an account': 'Já tenho uma conta',
	'New password': 'Nova senha',
	'If the email exists, you will receive the instructions.':
		'Se o email existir, você receberá as instruções.',
	'If this email is registered and unverified, we sent a new link. Check your inbox.':
		'Se este email estiver cadastrado e não verificado, enviamos um novo link. Confira sua caixa de entrada.',
	'Protect your account with an authenticator app.': 'Proteja sua conta com um app autenticador.',
	'{count} recovery codes remaining': '{count} recovery codes restantes',
	'{count} categories': '{count} categorias',
	'{count} payments': '{count} pagamentos',
	'{count} vendors': '{count} fornecedores',
	'{count} cost centers': '{count} centros de custo',
	'{shown} of {total} items shown': '{shown} de {total} itens exibidos',
	'{count} expense': '{count} despesa',
	'{count} expenses': '{count} despesas',
	'{count} recurrence': '{count} recorrência',
	'{count} recurrences': '{count} recorrências',
	'{expenseCount} expenses + {recurrenceCount} recurrences':
		'{expenseCount} despesas + {recurrenceCount} recorrências',
	'{pageSize} per page': '{pageSize} por página',
	'{start}-{end} of {total}': '{start}-{end} de {total}',
	'Page {page} of {count}': 'Página {page} de {count}',
	'Search {item}': 'Buscar {item}',
	'Search in {collection}': 'Buscar em {collection}',
	'Line {rowNumber}: {message}': 'Linha {rowNumber}: {message}',
	'{count} failure': '{count} falha',
	'{count} failures': '{count} falhas',
	'Every {unit}': 'Todo {unit}',
	'Every {interval} {unitPlural}': 'A cada {interval} {unitPlural}',
	week: 'semana',
	weeks: 'semanas',
	month: 'mês',
	months: 'meses',
	year: 'ano',
	years: 'anos',
	'First {limit}': 'Primeiros {limit}',
	'Approved expenses': 'Aprovadas',
	'Pending expenses': 'Pendentes',
	'Rejected expenses': 'Rejeitadas',
	'{value} reconciled': '{value} conciliado',
	'{value} open': '{value} em aberto',
	'{value} paid': '{value} pagas',
	'Top category': 'Maior categoria',
	Variation: 'Variação',
	'{percent}% used': '{percent}% usado',
	Alerts: 'Alertas',
	'Categories near or above the goal': 'Categorias próximas ou acima da meta',
	'Weekly evolution': 'Evolução semanal',
	'Ranking by category': 'Ranking por categoria',
	'Ranking by week': 'Ranking por semana',
	'Ranking by payment': 'Ranking por pagamento',
	of: 'de',
	Administrative: 'Administrativo',
	Accounting: 'Contabilidade',
	Taxes: 'Impostos',
	Employees: 'Funcionários',
	Payroll: 'Folha de pagamento',
	Supplies: 'Insumos',
	Cleaning: 'Limpeza',
	Rent: 'Aluguel',
	Utilities: 'Contas',
	Phone: 'Telefonia',
	Technology: 'Tecnologia',
	Maintenance: 'Manutenção',
	Logistics: 'Logística',
	Transport: 'Transporte',
	Inventory: 'Estoque',
	Purchases: 'Compras',
	Sales: 'Vendas',
	Legal: 'Jurídico',
	Banking: 'Bancário',
	Insurance: 'Seguros',
	Meals: 'Alimentação',
	Travel: 'Viagem',
	Documents: 'Documentos',
	Other: 'Outros',
	Others: 'Outros',

	'Account created. Sign in to continue.': 'Conta criada. Entre para continuar.',
	'Account created. Check your email to verify your account before signing in.':
		'Conta criada. Confira seu email para verificar sua conta antes de entrar.',
	'If the account exists and needs verification, we sent a new verification link.':
		'Se a conta existir e precisar de verificação, enviamos um novo link de verificação.',
	'Image compressed from {from} to {to}.': 'Imagem comprimida de {from} para {to}.',
	'Image is still larger than 2 MB after compression.':
		'A imagem ainda está acima de 2 MB após a compressão.',
	'Attachment not found.': 'Anexo não encontrado.',
	'Attachment type is not allowed.': 'Tipo de anexo não permitido.',
	'Budget not found.': 'Orçamento não encontrado.',
	'Category not found.': 'Categoria não encontrada.',
	'Category is invalid.': 'Categoria inválida.',
	'Check category data.': 'Confira os dados da categoria.',
	'Check auxiliary catalog.': 'Confira o cadastro auxiliar.',
	'Check budget data.': 'Confira os dados do orçamento.',
	'Check email and password.': 'Confira email e senha.',
	'Check email and role.': 'Confira email e papel.',
	'Check expense data.': 'Confira os dados da despesa.',
	'Check file and format.': 'Confira arquivo e formato.',
	'Check name, email and password.': 'Confira nome, email e senha.',
	'Check member and role.': 'Confira membro e papel.',
	'Check MFA code.': 'Confira o código MFA.',
	'Check payment data.': 'Confira os dados do pagamento.',
	'Check recurrence data.': 'Confira os dados da recorrência.',
	'Check review data.': 'Confira os dados da revisão.',
	'Check rule data.': 'Confira os dados da regra.',
	'Check workspace data.': 'Confira os dados do workspace.',
	'Credentials are invalid.': 'Credenciais inválidas.',
	'Expense not found.': 'Despesa não encontrada.',
	'Approved expense not found.': 'Despesa aprovada não encontrada.',
	'Filters are invalid.': 'Filtros inválidos.',
	'Invalid auxiliary catalog.': 'Cadastro auxiliar inválido.',
	'Invalid alert month.': 'Mês inválido para alertas.',
	'Invalid attachment.': 'Anexo inválido.',
	'Invalid budget alert delivery.': 'Envio de alerta de orçamento inválido.',
	'Invalid budget alert preference.': 'Preferência de alertas de orçamento inválida.',
	'Invalid budget.': 'Orçamento inválido.',
	'Invalid category.': 'Categoria inválida.',
	'Invalid decision.': 'Decisão inválida.',
	'Invalid expense.': 'Despesa inválida.',
	'Invalid invite or expired.': 'Convite inválido ou expirado.',
	'Invalid invitation.': 'Convite inválido.',
	'Invalid member.': 'Membro inválido.',
	'Invalid MFA code.': 'Código MFA inválido.',
	'Invalid recurrence.': 'Recorrência inválida.',
	'Invalid rule.': 'Regra inválida.',
	'Invalid theme.': 'Tema inválido.',
	'Invalid token or expired.': 'Token inválido ou expirado.',
	'Invalid token or password.': 'Token ou senha inválidos.',
	'Invalid workspace.': 'Workspace inválido.',
	'Language is invalid.': 'Idioma inválido.',
	'Currency is invalid.': 'Moeda inválida.',
	'Member not found.': 'Membro não encontrado.',
	'Invitation not found or expired.': 'Convite não encontrado ou expirado.',
	'MFA enabled.': 'MFA ativado.',
	'MFA is already enabled.': 'MFA já está ativo.',
	'Name cannot contain control characters.': 'Nome não pode conter caracteres de controle.',
	'Invalid date.': 'Data inválida.',
	'Password updated.': 'Senha atualizada.',
	'Passwords do not match.': 'As senhas não conferem.',
	'Payment must be at most 80 characters.': 'Pagamento deve ter no máximo 80 caracteres.',
	'Provide a valid email.': 'Informe um email válido.',
	'Provide the MFA code.': 'Informe o código MFA.',
	'Rule not found.': 'Regra não encontrada.',
	'Could not create the account.': 'Não foi possível criar a conta.',
	'Could not compress image. Try a smaller file.':
		'Não foi possível comprimir a imagem. Tente um arquivo menor.',
	'Could not save the catalog.': 'Não foi possível salvar o cadastro.',
	'Could not save the category.': 'Não foi possível salvar a categoria.',
	'Could not update the workspace.': 'Não foi possível atualizar o workspace.',
	'Delete category': 'Excluir categoria',
	'Permission denied.': 'Permissão insuficiente.',
	'Origin is invalid.': 'Origem inválida.',
	'Internal error.': 'Erro interno.',
	'Too many attempts. Try again after {time}.': 'Muitas tentativas. Tente novamente após {time}.',
	'Amount is required.': 'Valor obrigatório.',
	'Amount is invalid.': 'Valor inválido.',
	'Amount exceeds the maximum allowed.': 'Valor excede o máximo permitido.',
	'Amount must be greater than zero.': 'Valor deve ser maior que zero.',
	'End date must be greater than or equal to start date.':
		'Data final deve ser maior ou igual a data inicial.',
	'Maximum allowed range is 10 years.': 'Intervalo máximo permitido é de 10 anos.',
	'CSV file is empty.': 'Arquivo CSV vazio.',
	'CSV must contain date, description and amount columns.':
		'CSV precisa conter colunas de data, descrição e valor.',
	'Portable CSV exceeds the 1 MB import file limit. Narrow the report filters and try again.':
		'O CSV portável excede o limite de 1 MB para importação. Restrinja os filtros do relatório e tente novamente.',
	'Portable CSV export supports at most {count} expenses. Narrow the report filters and try again.':
		'A exportação de CSV portável permite no máximo {count} despesas. Restrinja os filtros do relatório e tente novamente.',
	'Portable CSV version is not supported.': 'A versão do CSV portável não é compatível.',
	'Line {rowNumber}: date, description or amount is invalid.':
		'Linha {rowNumber}: data, descrição ou valor inválido.',
	'No OFX transaction found.': 'Nenhum lançamento OFX encontrado.',
	'OFX transaction {rowNumber}: date or amount is invalid.':
		'Lançamento OFX {rowNumber}: data ou valor inválido.',
	'OFX transaction': 'Lançamento OFX',
	'Category not found and no default category was selected.':
		'Categoria não encontrada e nenhuma categoria padrão foi selecionada.',
	'Default category is invalid.': 'Categoria padrão inválida.',
	'File is required.': 'Arquivo obrigatório.',
	'File is larger than 1 MB.': 'Arquivo acima de 1 MB.',
	'Import at most {count} rows at a time.': 'Importe no máximo {count} linhas por vez.',
	'Attachment is larger than 2 MB.': 'Anexo acima de 2 MB.',
	'Attachment is empty.': 'Anexo vazio.',
	'Attachment file not found.': 'Arquivo do anexo não encontrado.',
	'Attachment file already exists.': 'Arquivo de anexo já existe.',
	'Attachment path is invalid.': 'Caminho de anexo inválido.',
	'MFA configuration is invalid.': 'Configuração MFA inválida.',
	'BETTER_AUTH_SECRET is not configured.': 'BETTER_AUTH_SECRET não configurado.',
	'{kind} not found.': '{kind} não encontrado.',
	'{kind} already exists.': '{kind} já existe.',
	'{kind} is invalid.': '{kind} inválido.',
	'{kind} must be at least 2 characters.': '{kind} deve ter pelo menos 2 caracteres.',
	'{kind} must be at most {count} characters.': '{kind} deve ter no máximo {count} caracteres.',
	'{kind} contains invalid characters.': '{kind} contém caracteres inválidos.',
	'{count} budget alerts sent to {sentCount} recipients.':
		'{count} alertas enviados para {sentCount} destinatários.',
	'{sentCount} budget alert recipients notified; {failedCount} deliveries will be retried.':
		'{sentCount} destinatários de alertas de orçamento notificados; {failedCount} envios serão tentados novamente.',
	'{count} due recurring expenses generated.': '{count} despesas recorrentes geradas.',
	'{count} expenses imported.': '{count} despesas importadas.',
	'Invite created: {url}': 'Convite criado: {url}',
	'Invite link created': 'Link de convite criado',
	'Invitation created and email sent.': 'Convite criado e email enviado.',
	'Invitation created. Email delivery will be retried automatically.':
		'O convite foi criado. O envio do email será tentado novamente automaticamente.',
	'A pending invitation already exists. Its link and role were kept unchanged.':
		'Já existe um convite pendente. O link e o papel foram mantidos sem alterações.',
	'Invitation link rotated and email sent.': 'Link do convite renovado e email enviado.',
	'Invitation link rotated. Email delivery will be retried automatically.':
		'O link do convite foi renovado. O envio do email será tentado novamente automaticamente.',
	'Copy link': 'Copiar link',
	'Copy all': 'Copiar tudo',
	'Copy failed. Select and copy the link manually.':
		'Não foi possível copiar. Selecione e copie o link manualmente.',
	'Copied!': 'Copiado!',
	'Scan this QR code with your authenticator app': 'Escaneie este QR code com seu app autenticador',
	'Disabling MFA will reduce your account security.':
		'Desativar o MFA reduzirá a segurança da sua conta.',
	'Remove member?': 'Remover membro?',
	'Remove {name} from the workspace? They will lose access immediately.':
		'Remover {name} do workspace? Eles perderão o acesso imediatamente.',
	'Skip to main content': 'Pular para o conteúdo principal',
	'You received an invite to access {workspace}. Open: {url}':
		'Você recebeu um convite para acessar {workspace}. Acesse: {url}',
	'Accept invite': 'Aceitar convite',
	'Invite to {workspace}': 'Convite para {workspace}',
	'Access the link to reset your password: {url}': 'Acesse o link para redefinir sua senha: {url}',
	'Password reset': 'Redefinição de senha',
	'Verify your email': 'Verifique seu email',
	'Verify your email before signing in. Check your inbox for the verification link.':
		'Verifique seu email antes de entrar. Confira sua caixa de entrada pelo link de verificação.',
	'We sent a new verification link. Check your inbox before signing in.':
		'Enviamos um novo link de verificação. Confira sua caixa de entrada antes de entrar.',
	'Wait 2 minutes before requesting another verification email.':
		'Aguarde 2 minutos antes de solicitar outro email de verificação.',
	'Verification email limit reached. If the email is not verified within 1 hour, this registration will expire.':
		'Limite de emails de verificação atingido. Se o email não for verificado em 1 hora, este cadastro será expirado.',
	'Your unverified registration expired. Create your account again.':
		'Seu cadastro não verificado expirou. Crie sua conta novamente.',
	'Access the link to verify your email: {url}': 'Acesse o link para verificar seu email: {url}',
	'Budget alerts for {workspace} in {period}:':
		'Alertas de orçamento para {workspace} em {period}:',
	'Budget alerts - {workspace}': 'Alertas de orçamento - {workspace}',
	'{spent} of {budget}': '{spent} de {budget}',
	'No baseline': 'Sem base',
	'A proposed category is no longer available.': 'Uma categoria proposta não está mais disponível.',
	'Cannot reject a paid or reconciled expense.':
		'Não é possível rejeitar uma despesa paga ou conciliada.',
	'Cannot change payment status of a reconciled expense.':
		'Não é possível alterar o status de pagamento de uma despesa conciliada.',
	'Cancel preview': 'Cancelar prévia',
	'Review status has changed. Reload and try again.':
		'Status de revisão alterado. Recarregue e tente novamente.',
	'Expense was modified. Reload and try again.':
		'Despesa foi alterada. Recarregue e tente novamente.',
	Viewer: 'Visualizador',
	Accepted: 'Aceito',
	Revoked: 'Revogado',
	Expired: 'Expirado',
	'My expenses': 'Minhas despesas',
	'Example payment': 'Ex.: Pix',
	'Example vendor': 'Ex.: ACME',
	'Example cost center': 'Ex.: Operações',
	'Something went wrong.': 'Algo deu errado.',
	'Go to dashboard': 'Ir para o início',
	'Recurring expense not found.': 'Recorrência não encontrada.',
	'{count} duplicates skipped.': '{count} duplicatas ignoradas.',
	'{count} duplicates': '{count} duplicatas',
	'{count} proposed': '{count} propostas',
	'{count} rows selected': '{count} linhas selecionadas',
	'{count} undone': '{count} desfeitas',
	'Imported expenses undone: {undoneCount}. Protected expenses skipped: {skippedCount}.':
		'Despesas importadas desfeitas: {undoneCount}. Despesas protegidas ignoradas: {skippedCount}.',
	'Clear selection': 'Limpar seleção',
	'Confirm selected expenses': 'Confirmar despesas selecionadas',
	'Confirmed import batch is unavailable.': 'O lote de importação confirmado não está disponível.',
	Duplicate: 'Duplicata',
	'Include row {rowNumber}': 'Incluir linha {rowNumber}',
	'Proposed category': 'Categoria proposta',
	Ready: 'Pronta',
	'Review ledger': 'Livro de revisão',
	'Select proposed': 'Selecionar propostas',
	'Source row': 'Linha de origem',
	'Undo import': 'Desfazer importação',
	'Undo this import batch?': 'Desfazer este lote de importação?',
	'Send alerts now': 'Enviar alertas agora',
	'Send budget alert email': 'Enviar alertas de orçamento por email',
	'Budget alert email already sent for this month.':
		'Alertas de orçamento já enviados para este mês.',
	'Delete attachment?': 'Excluir comprovante?',
	'Bulk actions': 'Ações em lote',
	'{count} selected': '{count} selecionado(s)',
	'Select {description}': 'Selecionar {description}',
	'You cannot change your own role.': 'Você não pode alterar seu próprio papel.',
	'You cannot remove yourself.': 'Você não pode remover a si mesmo.',
	'Cannot change currency while this workspace has monetary records. Remove or finish them first.':
		'Não é possível alterar a moeda enquanto este workspace tiver registros monetários. Remova-os ou conclua-os primeiro.',
	'Bank ledger': 'Livro bancário',
	'Bank transaction ignored.': 'Lançamento bancário ignorado.',
	'Bank transaction currency does not match the workspace currency.':
		'A moeda do lançamento bancário não corresponde à moeda do workspace.',
	'Bank transaction matched and reconciled.': 'Lançamento bancário associado e conciliado.',
	'Bank transaction not found.': 'Lançamento bancário não encontrado.',
	'Bank transaction was already decided.': 'O lançamento bancário já foi decidido.',
	'Category for new expense': 'Categoria da nova despesa',
	Credit: 'Crédito',
	'Credit transactions can only be ignored.': 'Lançamentos de crédito só podem ser ignorados.',
	'Credits are staged for visibility and cannot create or match expenses.':
		'Créditos são preparados para visibilidade e não podem criar nem associar despesas.',
	'Create and reconcile': 'Criar e conciliar',
	Debit: 'Débito',
	'Exact amount': 'Valor exato',
	'Exact amount candidates': 'Candidatas com valor exato',
	'Expense created and reconciled.': 'Despesa criada e conciliada.',
	'Expense is no longer eligible for this match.':
		'A despesa não está mais elegível para esta associação.',
	Ignore: 'Ignorar',
	Match: 'Associar',
	'No eligible expense found within the date window.':
		'Nenhuma despesa elegível encontrada no intervalo de datas.',
	'OFX transaction {rowNumber}: date, amount or description is invalid.':
		'Lançamento OFX {rowNumber}: data, valor ou descrição inválidos.',
	'OFX statement currency is invalid.': 'A moeda do extrato OFX é inválida.',
	'OFX staging failures': 'Falhas na preparação do OFX',
	'Reconcile OFX transactions': 'Conciliar lançamentos OFX',
	'Reconciliation choice is invalid.': 'A escolha de conciliação é inválida.',
	'Same date': 'Mesma data',
	'Search description': 'Buscar descrição',
	'Search staged transactions': 'Buscar lançamentos preparados',
	'Select category': 'Selecione a categoria',
	'Suggestions never change expenses until you confirm a decision.':
		'Sugestões nunca alteram despesas até que você confirme uma decisão.',
	'This statement currency does not match the workspace. You can only ignore this transaction.':
		'A moeda deste extrato não corresponde à do workspace. Você só pode ignorar este lançamento.',
	'This membership cannot be changed through an invitation.':
		'Esta associação não pode ser alterada por meio de um convite.',
	'This user is already an active workspace member.':
		'Este usuário já é um membro ativo do workspace.',
	'{count} days apart': '{count} dias de diferença',
	'{count} transactions shown': '{count} lançamentos exibidos',
	'{score}% description overlap': '{score}% de semelhança na descrição',
	'{stagedCount} bank transactions staged; {duplicateCount} duplicates and {failedCount} failures.':
		'{stagedCount} lançamentos bancários preparados; {duplicateCount} duplicatas e {failedCount} falhas.',
	'A newer recurring expense already exists for this date.':
		'Já existe uma despesa recorrente mais recente para esta data.',
	'An attachment is corrupted and cannot be restored.':
		'Um comprovante está corrompido e não pode ser restaurado.',
	'An attachment is no longer available for restore.':
		'Um comprovante não está mais disponível para restauração.',
	'Back to expenses': 'Voltar para despesas',
	'Delete expense permanently?': 'Excluir despesa permanentemente?',
	'Delete permanently': 'Excluir permanentemente',
	'Deleted expenses': 'Despesas excluídas',
	'Deleted expenses remain recoverable for 30 days.':
		'Despesas excluídas permanecem recuperáveis por 30 dias.',
	'Deleted expenses will appear here during their recovery period.':
		'Despesas excluídas aparecerão aqui durante o período de recuperação.',
	'Deleted {date}': 'Excluída em {date}',
	'Expense trash': 'Lixeira de despesas',
	'Permanent deletion {date}': 'Exclusão permanente em {date}',
	'Only workspace managers can recover this expense.':
		'Somente gestores do workspace podem recuperar esta despesa.',
	Recovery: 'Recuperação',
	'Recovery period expired': 'Período de recuperação expirado',
	'Restore the expense category before restoring this expense.':
		'Restaure a categoria da despesa antes de restaurar esta despesa.',
	'Restore the referenced {kind} before restoring this expense.':
		'Restaure {kind} antes de restaurar esta despesa.',
	'The expense will move to trash and remain recoverable for 30 days.':
		'A despesa será movida para a lixeira e poderá ser recuperada por 30 dias.',
	'The workspace currency changed, so this expense cannot be restored.':
		'A moeda do workspace mudou, portanto esta despesa não pode ser restaurada.',
	'This action cannot be undone.': 'Esta ação não pode ser desfeita.',
	'This expense can no longer be restored.': 'Esta despesa não pode mais ser restaurada.',
	'This expense is still within its recovery period.':
		'Esta despesa ainda está no período de recuperação.',
	'Trash cursor is invalid.': 'O cursor da lixeira é inválido.',
	'Trash is empty': 'A lixeira está vazia',
	'Trashed expense not found.': 'Despesa na lixeira não encontrada.',
	'View trash': 'Ver lixeira',
	'All actions': 'Todas as ações',
	'All entities': 'Todas as entidades',
	'Attachment deleted': 'Comprovante excluído',
	'Attachment uploaded': 'Comprovante enviado',
	'Bank statement': 'Extrato bancário',
	'Bank statement staged': 'Extrato bancário preparado',
	'Bank transaction': 'Lançamento bancário',
	'Bank transaction created': 'Lançamento bancário criado',
	'Bank transaction ignored': 'Lançamento bancário ignorado',
	'Bank transaction matched': 'Lançamento bancário conciliado',
	'Bank transaction reconciliation reversed': 'Conciliação do lançamento bancário revertida',
	'Budget alert delivery': 'Envio de alerta de orçamento',
	'Budget alert delivery retried': 'Nova tentativa de envio de alerta de orçamento',
	'Budget alert preference': 'Preferência de alerta de orçamento',
	'Budget alerts disabled': 'Alertas de orçamento desativados',
	'Budget alerts enabled': 'Alertas de orçamento ativados',
	'Budget alerts sent': 'Alertas de orçamento enviados',
	'Budget deleted': 'Orçamento excluído',
	'Budget saved': 'Orçamento salvo',
	'Catalog item archived': 'Item de cadastro arquivado',
	'Catalog item deleted': 'Item de cadastro excluído',
	'Catalog item saved': 'Item de cadastro salvo',
	'Catalog item updated': 'Item de cadastro atualizado',
	'Category archived': 'Categoria arquivada',
	'Category budget': 'Orçamento de categoria',
	'Category created': 'Categoria criada',
	'Category deleted': 'Categoria excluída',
	'Category restored': 'Categoria restaurada',
	'Category rule': 'Regra de categoria',
	'Category rule archived': 'Regra de categoria arquivada',
	'Category rule created': 'Regra de categoria criada',
	'Category updated': 'Categoria atualizada',
	'Expense approved': 'Despesa aprovada',
	Expense: 'Despesa',
	'Expense attachment': 'Comprovante da despesa',
	'Expense created': 'Despesa criada',
	'Expense deleted': 'Despesa excluída',
	'Expense import completed': 'Importação de despesas concluída',
	'Expense import failed': 'Importação de despesas falhou',
	'Expense import undone': 'Importação de despesas desfeita',
	'Expense installments created': 'Parcelas da despesa criadas',
	'Expense marked as paid': 'Despesa marcada como paga',
	'Expense marked as unpaid': 'Despesa marcada como não paga',
	'Expense permanently deleted': 'Despesa excluída permanentemente',
	'Expense reconciled': 'Despesa conciliada',
	'Expense rejected': 'Despesa rejeitada',
	'Expense restored': 'Despesa restaurada',
	'Expense updated': 'Despesa atualizada',
	'Expenses approved in bulk': 'Despesas aprovadas em lote',
	'Expenses rejected in bulk': 'Despesas rejeitadas em lote',
	'Import batch': 'Lote de importação',
	'MFA disabled': 'MFA desativado',
	'MFA enabled': 'MFA ativado',
	'Recurring expense': 'Despesa recorrente',
	'Recurring expense created': 'Despesa recorrente criada',
	'Recurring expense paused': 'Despesa recorrente pausada',
	'Recurring expense resumed': 'Despesa recorrente retomada',
	'Recurring expenses generated': 'Despesas recorrentes geradas',
	'View technical metadata': 'Ver metadados técnicos',
	'Workspace created': 'Workspace criado',
	'Workspace invitation': 'Convite do workspace',
	'Workspace invitation accepted': 'Convite do workspace aceito',
	'Workspace invitation resent': 'Convite do workspace reenviado',
	'Workspace member': 'Membro do workspace',
	'Workspace member disabled': 'Membro do workspace desativado',
	'Workspace member invited': 'Membro do workspace convidado',
	'Workspace member role changed': 'Papel do membro do workspace alterado',
	'Workspace updated': 'Workspace atualizado',
	'Child categories': 'Categorias filhas',
	Count: 'Quantidade',
	Created: 'Criados',
	Decision: 'Decisão',
	'Delivery model': 'Modelo de envio',
	Duplicates: 'Duplicatas',
	'Escalate over budget': 'Escalar acima do orçamento',
	'Expired at': 'Expirado em',
	'File size (bytes)': 'Tamanho do arquivo (bytes)',
	'File type': 'Tipo de arquivo',
	'Generated through': 'Gerado até',
	'Match target': 'Alvo da correspondência',
	'Paid at': 'Pago em',
	'Posted date': 'Data do lançamento',
	'Recipient mode': 'Modo de destinatários',
	'Restored attachments': 'Comprovantes restaurados',
	Rows: 'Linhas',
	'Signed amount (cents)': 'Valor com sinal (centavos)',
	Skipped: 'Ignorados',
	'Source type': 'Tipo de origem',
	Undone: 'Desfeitos'
} as const;
